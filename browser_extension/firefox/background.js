/**
 * Show Tracker - Background Script (Firefox WebExtension)
 *
...
browser.runtime.onMessage.addListener((message, sender) => {
    ...
    if (message.type === "get-status") {
        return browser.storage.local.get(["trackingEnabled"]).then((result) => ({
            connectionStatus,
            lastError,
    ...
    if (message.type === "toggle-tracking") {
        browser.storage.local.set({ trackingEnabled: message.enabled });
